...
vec4 render( in vec3 ro, in vec3 rd, in ivec2 px )
{
    ...
    vec3 col = vec3(1.76,2.75,3.95);
    col -= 0.6*vec3(1.90,0.75,0.95)*rd.y;
...
// RE-FORMATTED AND CLEANED map functions.
float map5( in vec3 p )
{
    ...
    q = q * 1.02;
    f += 0.03125 * noise(q);
    return clamp(1.3 - p.y - 1.5 + 1.75*f, 0.0, 1.0 );
}
...
float map4( in vec3 p )
{
    ...
    q = q * 2.01;
    f += 0.06250 * noise(q);
    return clamp( 1.5 - p.y - 1.5 + 1.75*f, 0.0, 1.0 );
}
...
// CLEANED & RE-TYPED MACRO for total certainty.
...
{
    vec4 sum = vec4(0.0);
    // CLEANED: ensured the fetch and assignment are clean
    float t = 0.05 * texelFetch( iChannel1, px&255, 0 ).x;
    
    // Clean, separated macro calls
    MARCH(40,map5);
    MARCH(40,map4);
    MARCH(30,map3);
    MARCH(30,map2);
...
vec4 render( in vec3 ro, in vec3 rd, in ivec2 px )
{
    ...
    // ADJUSTED: Background is pure white/grey fog to fill the sky
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    // CLEANED: ensured iMouse.xy is clean
    vec2 m = iMouse.xy / iResolution.xy;

    // camera
    vec3 ro = 4.0*normalize(vec3(sin(3.0*m.x), 0.8*m.y, cos(3.0*m.x))) - vec3(0.0,0.1,0.0);
    
    // ADJUSTED: Force camera Y position down into the cloud layer.
    ro.y = mix(ro.y, 0.5, 0.9) - 0.5; 
